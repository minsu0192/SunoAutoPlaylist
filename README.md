...
~/SunoProjects/input/집에가고싶다.jpg
        ↓  (파일명 = 키워드)
Claude API (1회/곡, 성과 피드백 반영)
  → 가사 + Suno 스타일 + 곡 제목
  → YouTube 제목 + 설명 (참고 채널 스타일 반영)
        ↓
Playwright (기본) / pyautogui (fallback)
  → suno.com/create 자동 조작
  → CSS 셀렉터 기반 (해상도 독립적)
  → 가사/스타일/제목 입력 → Create 클릭
  → 생성 대기 (~2분) → MP3 자동 다운로드
        ↓
FFmpeg → 커버이미지 + MP3 → MP4
        ↓
YouTube Data API → 자동 업로드
  → 성과 데이터 수집 → 다음 곡 생성에 피드백 반영
```
...
1. 메뉴바 → ▶ 지금 실행
2. **Playwright**로 suno.com/create 자동 이동 (세션 쿠키 유지)
3. CSS 셀렉터로 Advanced 탭 → 가사/스타일/제목 자동 입력 → Create 클릭
4. 생성 대기 (~2분) → audio 태그에서 MP3 URL 감지 → 다운로드
5. Playwright 실패 시 **pyautogui 좌표 방식으로 자동 fallback**
6. FFmpeg로 MP3 + 커버이미지 → MP4
7. YouTube 자동 업로드 (설정된 플레이리스트에 추가)
8. 업로드 기록 저장 → 성과 분석 → 다음 곡 생성에 반영

> Suno 첫 실행 시 브라우저에서 수동 로그인 필요 (이후 세션 자동 유지)

---

## YouTube 성과 분석 (피드백 루프)

업로드된 영상의 조회수/좋아요 데이터를 자동 수집하여 다음 곡 생성에 반영합니다.

```bash
python youtube_analytics.py                  # 전체 분석 실행
python youtube_analytics.py --collect-only   # 데이터 수집만
python youtube_analytics.py --analyze-only   # 분석만
```
...
| 증상 | 해결 |
|------|------|
| Playwright 브라우저 오류 | `playwright install chromium` |
